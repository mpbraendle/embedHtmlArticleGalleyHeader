...
class EmbedHtmlArticleGalleyHeaderPlugin extends GenericPlugin
{
        ...
        if (parent::register($category, $path, $mainContextId)) {
            if ($this->getEnabled($mainContextId)) {
                HookRegistry::register(
                  ...
                  array($this, 'articleViewCallback'),
                  HOOK_SEQUENCE_LATE
                );
        ...
        if ($submissionFile->getData('mimetype') === 'text/html') {
            ...
            if (!HookRegistry::call('HtmlArticleGalleyPlugin::articleDownload', array($article, &$galley, &$fileId))) {
                            ...
                            foreach ($links as $l) {
                                if ($l->getAttribute("rel") == "stylesheet") {
                                    $templateMgr->addHeader(
                                      'embedStylesheet' . $count . '',
                                      '<link rel="stylesheet" type="text/css" href="' . $l->getAttribute("href") . '">'
                                    );
                                    $count++;
                                }
                            }
                        ...
                        if ($head->getElementsByTagName('script')->length != 0) {
                            ...
                            foreach ($scripts as $script) {
                                if (stristr($script->getAttribute("src"), '.js')) {
                                    $templateMgr->addHeader(
                                      'embedJs' . $count . '',
                                      '<script type="text/javascript" src="' . $script->getAttribute(
                                        "src"
                                      ) . '"></script>'
                                    );
                                    $count++;
                ...
                $templateMgr->assign(array(
                  ...
                  'article' => $article,
                  'galley' => $galley,
                  'isLatestPublication' => $article->getData('currentPublicationId') === $galley->getData(
                      'publicationId'
                    ),
    ...
    protected function _getHTMLContents($request, $galley)
    {
        ...
        foreach ($embeddableFiles as $embeddableFile) {
            ...
            if ($embeddableFile->getData('mimetype') == 'text/plain' || $embeddableFile->getData(
                'mimetype'
              ) == 'text/css') {
                $params['inline'] = 'true';
            }
            ...
            $fileUrl = $request->url(
              ...
              'article',
              'download',
              array($referredArticle->getBestArticleId(), $galley->getBestGalleyId(), $embeddableFile->getId()),
              $params
            );
        ...
        $paramArray = array(
          'issueTitle' => $issue ? $issue->getIssueIdentification() : __(
    ...
    function _handleOjsUrl($matchArray)
    {
        ...
        if (isset($urlParts[0])) switch (strtolower_codesafe($urlParts[0])) {
            ...
            case 'sitepublic':
                ...
                $url = $request->getBaseUrl() . '/' . $publicFileManager->getSiteFilesPath() . '/' . implode(
                    '/',
                    $urlParts
                  ) . ($anchor ? '#' . $anchor : '');
                break;
            case 'public':
                ...
                import('classes.file.PublicFileManager');
                $publicFileManager = new PublicFileManager();
                $url = $request->getBaseUrl() . '/' . $publicFileManager->getJournalFilesPath(
                    $journal->getId()
                  ) . '/' . implode('/', $urlParts) . ($anchor ? '#' . $anchor : '');
                break;
        }
